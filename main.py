...
CONCRETE_HINT_RUBRIC = (
    "Выбери сам одну узкую практическую подтему внутри рубрики (без повтора предыдущих). "
    "Добавь конкретики: 1 пример-расчёт в ₽ и блок **🧩 Шаги:** (3–5 пунктов по 1 строке)."
    "Не используй тему «финансовая подушка безопасности», если она не задана явно."
)
CONCRETE_HINT_NEWS = (
...
def test_rubric_post(rubric_name):
    ...
    while attempts < 5:
        text = generate_post_text(
            f"Создай структурированный и интересный Telegram-пост по рубрике: {rubric_name}. {CONCRETE_HINT_RUBRIC}",
